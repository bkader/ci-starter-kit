...
if ( ! function_exists('config')) {
     ...
     * @return  mixed
     */
    function config($item = null, $default = false)
    {
        // We check if the method MY_Config::get() exists
        if (method_exists(get_instance()->config, 'get'))
        {
            return get_instance()->config->get($item, $default);
        }
        // In case it does not, we check if the dot() function exitst
        elseif (function_exists('dot'))
        {
            global $CFG;
            return dot($CFG->config, $item, $default);
        }
        // Otherwise, we simply reply on Ci_Config::get() method
        else
        {
...
if ( ! function_exists('load_config')) {
    /**
     * Load a configuration file even if CI is not available
    ...
    function &load_config($file = null)
    {
        ...
        if (file_exists($file_path)) {
            ...
            // Does the config array exists
            if ( ! isset($config) OR ! is_array($config)) {
                set_status_header(503);
                echo 'Your '.$file.' file does not appear to be formatted correctly.';
                exit(3); // EXIT_CONFIG
            }
        }
